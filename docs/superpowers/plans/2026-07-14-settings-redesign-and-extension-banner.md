...
- **Colour rule.** Every *semantic* colour comes from the custom properties already in `frontend/src/index.css` (`--accent`, `--accent-hover`, `--accent-light`, `--stripe-primary`, `--radius-pill`, `--border`, `--border-light`, `--text`, `--text-secondary`, `--text-muted`, `--bg-white`, `--bg-page`, `--stripe-ruby`). Raw `rgba()` is allowed **only** for translucent washes of a colour that is already on screen (white over the banner gradient; ink over the nav; ruby over the danger row). Two literal exceptions are deliberate and must not be "fixed" into tokens:
  1. The banner gradient stops `#665efd → #533afd → #4434d4`, mirroring the existing `--stripe-cta-gradient` family.
  2. The Verified pill's success green `#0f7a4d` on `rgba(16, 133, 85, 0.1)`. **`index.css` has no success token** — it defines accent, ink, ruby and magenta only. This introduces the colour locally rather than inventing a global token as a side effect of a settings redesign. If a success token is added later, swap it.
