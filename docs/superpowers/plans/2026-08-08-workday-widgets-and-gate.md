...
> **Correction — 2026-08-09 (added after implementation).** Everything below describing `MULTISELECT_ID_ATTR` and the `data-uxi-multiselect-id` branch in `isMultiSelect` **was implemented and then deliberately reverted**, in commit `3fbc19a` (*revert(extension): data-uxi-multiselect-id is not a multi-select signal*). The premise was wrong: the very input that motivated it, `education-11--fieldOfStudy`, is a **single-value** control that happens to carry the marker, and the repo's only captured production Workday DOM (`chrome-extension/test/fixtures/workdayReal.ts:19-23`, Country Phone Code) shows the same marker on another single-value control. Measured cost: `"Toronto, Ontario"` committed `"Ontario"` and still reported `filled: true` — a wrong value banked as success. Measured benefit: none reachable, because real Workday multiselects nest inside `multiselectInputContainer` / `multiSelectContainer`, which the pre-existing `closest('[data-automation-id*="multiselect" i]')` already matches. `MULTISELECT_ID_ATTR` was therefore deleted from `workdaySelectors.ts` and does not exist in the shipped code. The **`isTypeahead` / `SEARCH_BOX_FRAGMENT` half of §1e was kept** and did ship. Evidence: `.superpowers/sdd/2026-08-08-workday-widgets-and-gate/task-3-report.md` § *"Fix round 2 — the marker branch reverted"*, and the `Task 3: ADJUDICATION` entry in that directory's `progress.md`. The original text is left intact below on purpose — the design as-planned, and why it did not survive contact with the DOM, are both worth keeping.
