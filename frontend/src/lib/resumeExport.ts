...
export function printResume(node: HTMLElement, pageSize: Theme["page_size"] = "letter") {
  ...
  win.document.write(
    `<!doctype html><html><head><meta charset="utf-8"><title>Resume</title>` +
      ...
      // Strip every on-screen highlight layer (keyword heatmap + "what changed"
      // underline + figure flags) so downloads never carry highlights.
      `mark{background:transparent !important;border:0 !important;box-shadow:none !important;` +
      `text-decoration:none !important;padding:0 !important;}</style>` +
